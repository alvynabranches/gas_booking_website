<?php
    ...
    function login(){
        if(!isset($_COOKIE['PHPSESSID'])){
            session_start();
        }if(isset($_POST['username'])&&isset($_POST['password'])){$user=$_POST['username'];$pwd=$_POST['password'];$result=exec_query("select customer_id as id, customer_address as ca, customer_no as cpn, customer_email as ce, customer_type as ct, customer_location_id as clid, customer_name as cn, password as pd from customer where username='$user';");
            if($result->num_rows==1){
                while($row=mysqli_fetch_assoc($result)){
                    $id=$row['id'];$cn=$row['cn'];$pd=$row['pd'];$ct=$row['ct'];$cl=$row['clid'];$cpn=$row['cpn'];$ce=$row['ce'];$ca=$row['ca'];
                    if(REPORT){
                ...
                $_SESSION['type']=$ct;
                $_SESSION['customer_location_id']=$cl;
    ...
    function booking(){
        if(isset($_POST['amount'])&&isset($_POST['book_now'])&&isset($_POST['payment_option'])){
            if($_POST['book_now'] == '1'){$amount=$_POST['amount'];$payment_option=$_POST['payment_option'];
                ...
                if(exec_query("INSERT INTO booking(booking_date, booking_amount, booking_customer_id, booking_status, booking_type) VALUES ('$date_time_now', '$amount', '$c_id', 'pending', '$payment_option');")===TRUE){
                    if(REPORT){console_log('Booking Successfully Done!');}alert('Booking Successfully Done!');
                }else{if(REPORT){console_log('Booking Unsuccessful!');}alert('Booking Unsuccessful!');}}}}
    ...
    function get_location($location_id){
        ...
        if($result->num_rows==1){while($row=mysqli_fetch_assoc($result)){$ln=$row['ln'];}}
        return $ln;
    }
