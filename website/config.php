<?php
    ...
    function login(){
        ...
        if(isset($_POST['username'])&&isset($_POST['password'])){
            $user=$_POST['username'];$pwd=$_POST['password'];$sql="select customer_id as id, customer_type as ct, customer_name as cn, password as pd from customer where username='$user';";$result=exec_query($sql);
            if($result->num_rows==1){while($row=mysqli_fetch_assoc($result)){
                $id=$row['id'];
                $cn=$row['cn'];
                $pd=$row['pd'];
                $ct=$row['ct'];
                if(REPORT){console_info('Username Found!');}}}
                else{
                    if(REPORT){console_warn('User Not Found!');}}$password_check=password_verify($pwd, $pd);
                    if($password_check){
                        $_SESSION['id']=$id;
                        $_SESSION['name']=$id;
                        $_SESSION['username']=$user;
                        header('Location: user.php');redirect('user.php', TRUE);if(REPORT){console_info($_SESSION['id']);console_info($_SESSION['name']);console_info($_SESSION['username']);}if(REPORT){console_log('Logged In Successfully!');}}else{if(REPORT){console_warn('Password Incorrect!');}echo "<script>alert('Password Incorrect!');</script>";}}}
    ...
    function booking(){
        if(isset($_POST['amount'])&&isset($_POST['book_now'])){
            if($_POST['book_now'] == '1'){
                $amount=$_POST['amount'];
