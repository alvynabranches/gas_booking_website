<?php
    ...
    function login(){if(!isset($_COOKIE['PHPSESSID'])){session_start();}if(isset($_POST['username'])&&isset($_POST['password'])){$user=$_POST['username'];$pwd=$_POST['password'];$sql="select customer_id as id, customer_address as ca, customer_no as cpn, customer_email as ce, customer_type as ct, customer_location_id as clid, customer_name as cn, password as pd from customer where username='$user';";$result=exec_query($sql);if($result->num_rows==1){while($row=mysqli_fetch_assoc($result)){$id=$row['id'];$cn=$row['cn'];$pd=$row['pd'];$ct=$row['ct'];$cl=$row['clid'];$cpn=$row['cpn'];$ce=$row['ce'];$ca=$row['ca'];if(REPORT){console_info('Username Found!');}}}else{if(REPORT){console_warn('User Not Found!');}}$password_check=password_verify($pwd, $pd);if($password_check){$_SESSION['id']=$id;$_SESSION['name']=$id;$_SESSION['username']=$user;$_SESSION['type']=$ct;$_SESSION['customer_location_id']=$cl;$_SESSION['customer_phone_no']=$cpn;$_SESSION['customer_address']=$ca;header('Location: user.php');redirect('user.php', TRUE);if(REPORT){console_info($_SESSION['id']);console_info($_SESSION['name']);console_info($_SESSION['username']);}if(REPORT){console_log('Logged In Successfully!');}}else{if(REPORT){console_warn('Password Incorrect!');}echo "<script>alert('Password Incorrect!');</script>";}}}
    ...
    function booking(){
        if(isset($_POST['amount'])&&isset($_POST['book_now'])&&isset($_POST['payment_option'])){
            if($_POST['book_now'] == '1'){
                $amount=$_POST['amount'];$payment_option=$_POST['payment_option'];$c_id=$_SESSION['id'];$date_time_now=date('Y-m-d H:i:s');
                $sql="insert into booking(booking_date, booking_amount, booking_customer_id, booking_status, booking_type) values ('$time_now', '$amount', '$c_id', 'pending', '$payment_option');";
            }
        }
    }
