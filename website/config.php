<?php
    ...
    function booking(){if(isset($_POST['amount'])&&isset($_POST['book_now'])&&isset($_POST['payment_option'])){if($_POST['book_now'] == '1'){$amount=$_POST['amount'];$payment_option=$_POST['payment_option'];$c_id=$_SESSION['id'];$date_time_now=date('Y-m-d H:i:s');if(exec_query("insert into booking(booking_date, booking_amount, booking_customer_id, booking_status, booking_type) values ('$time_now', '$amount', '$c_id', 'pending', '$payment_option');")===TRUE){console_log('Booking Successfully Done!');echo "<script>alert('Booking Successfully Done!');</script>";}else{console_log('Booking Unsuccessfully!');echo "<script>alert('Booking Unsuccessfully!');</script>";}}}}
        ...
        if(isset($_POST['old_password'])&&isset($_POST['new_password'])&&isset($_POST['confirm_new_password'])){
            ...
            if $np==$cnp{
                ...
                if(password_verify($op, $db_pwd)){
                    ...
                    exec_query("UPDATE TABLE customer SET password='$pwd' where id='$id'");
                }
            }
        }
    }
